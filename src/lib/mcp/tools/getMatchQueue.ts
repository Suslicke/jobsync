...
export const McpGetMatchQueueInputShape = {
  limit: z.number().int().min(1).max(5).optional().describe("How many jobs to return (default 3, max 5)."),
};
...
export async function handleGetMatchQueue(input: z.infer<typeof McpGetMatchQueueSchema>, userId: string) {
  ...
  // save_match_result only accepts MCP-created jobs, so the queue is limited to those.
  const where = {
    userId,
    matchScore: null,
    createdVia: { not: null },
    descriptionCompleteness: { in: ["partial", "full"] },
  };
  const [remaining, jobs] = await Promise.all([
    ...
    prisma.job.findMany({
      ...
      take: input.limit ?? 3,
      select: {
        id: true, description: true, descriptionCompleteness: true, jobUrl: true,
  ...
  const parts = jobs.map((j, i) =>
    `=== JOB ${i + 1}/${jobs.length}: ${j.Company.label} — ${j.JobTitle.label} (status: ${j.Status.label})\n` +
    `jobId: ${j.id}${j.jobUrl ? `\nurl: ${j.jobUrl}` : ""}\n\n` +
    `JOB DESCRIPTION:\n${j.description}\n\n` +
    buildMatchDirective(j.id, resume.id!, pre.data.normalizedText, j.descriptionCompleteness as DescriptionCompleteness, "update"),
  );
  return text(
    `${remaining} job(s) without a match score; returning ${jobs.length}. ` +
