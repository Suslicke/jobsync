...
export const MCP_ACTIONS: Record<string, McpActionEntry> = {
  ...
  "company.setCompanyHidden": { fn: m12.setCompanyHidden as any, signature: "companyId: string, hidden: boolean,", params: [{"name":"companyId","type":"string","optional":false},{"name":"hidden","type":"boolean","optional":false}], doc: "Rule an employer out wholesale, or let it back in. Deleting the company is not the same decision and is usually impossible anyway: its jobs are still needed so the collectors do not offer them again as new. Hiding leaves every row where it is and takes the employer out of the job list, which is what", schemas: {  } },
